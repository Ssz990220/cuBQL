...
namespace cuBQL {
  namespace lineSegs {
    ...
    /*! compute point on 'segment' that is closest to 'queryPoint',
      and return the square distance to that point. */
    inline __device__
    CPResult findClosestPoint(const vec3f queryPoint, const Segment segment)
    {
      CPResult result;
      ...
      result.sqrDistance = sqrDistance(queryPoint,segment.begin);
      return result;
    }
             ...
             const vec3f          *const __restrict__ vertices)
    {
      ...
      while (true) {
        ...
        for (int i=0;i<count;i++) {
          ...
          CPResult primResult = findClosestPoint(queryPoint,seg);
